...
def ask_viz_question(df: pd.DataFrame, user_question: str) -> t.List:
    question = f"""
    ...
    try:
        ...
        return [dp.Text(f"```Exception: {e}```\nVega-spec:```{vega_spec}```")]

    return [
        plot,
        dp.Toggle(dp.Code(vega_spec, language="json"), label="Show vega-lite spec"),
    ]


def ask_data_question(df: pd.DataFrame, user_question: str) -> t.List:
    con = duckdb.connect()
    ...
    return [dp.DataTable(sample), dp.Toggle(dp.Code(generated_query, language="sql"), label="Show SQL Query")]
